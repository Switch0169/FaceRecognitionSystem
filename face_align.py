...
if __name__ == '__main__':
    ...
    parser.add_argument("-source_root", "--source_root", help = "specify your source dir", default = "./FaceDatabase/Face", type = str)
    parser.add_argument("-dest_root", "--dest_root", help = "specify your destination dir", default = "./FaceDatabase/Face_Aligned", type = str)
    ...
    for subfolder in tqdm(os.listdir(source_root)):
        if not os.path.isdir(os.path.join(dest_root, subfolder).replace('\\', '/')):    # windows下出现的问题
            os.mkdir(os.path.join(dest_root, subfolder))
        for image_name in os.listdir(os.path.join(source_root, subfolder).replace('\\', '/')):
            print("Processing\t{}".format(os.path.join(source_root, subfolder, image_name).replace('\\', '/')))
            img = Image.open(os.path.join(source_root, subfolder, image_name).replace('\\', '/'))
            try: # Handle exception
                _, landmarks = detect_faces(img)
            except Exception:
                print("{} is discarded due to exception!".format(os.path.join(source_root, subfolder, image_name).replace('\\', '/')))
                continue
            if len(landmarks) == 0: # If the landmarks cannot be detected, the img will be discarded
                print("{} is discarded due to non-detected landmarks!".format(os.path.join(source_root, subfolder, image_name).replace('\\', '/')))
                continue
